...
def train_test(F, G, dset_loaders, criterion, epoch, phase, optimizers, args, assets, logger, use_gpu):
    ...
    print('')
    for batch_idx, (video_samples, mixed_spectrograms, speech_spectrograms) in \
        ...
        if use_gpu:
            if phase == 'train':
                video_samples, mixed_spectrograms, speech_spectrograms = \
                    Variable(video_samples.cuda()), Variable(mixed_spectrograms.cuda()), \
                    Variable(speech_spectrograms.cuda())
            if phase == 'val' or phase == 'test':
                with torch.no_grad():
                    video_samples, mixed_spectrograms, speech_spectrograms = \
                        Variable(video_samples.cuda()), Variable(mixed_spectrograms.cuda()), \
        ...
        Fx = F(video_samples, mixed_spectrograms)
        v_GFx, a_GFx = G(Fx)
        v_Gy, a_Gy = G(speech_spectrograms)
        FGy = F(v_Gy, a_Gy)
        ...
        l_nn = criterion(a_GFx, mixed_spectrograms) + criterion(v_GFx, video_samples)
        l_cn = criterion(a_Gy, mixed_spectrograms) + criterion(v_Gy, video_samples)
        l_cc = criterion(FGy, speech_spectrograms)
        loss_F = l_nc + l1*l_cc
        loss_G = l3 * (l_nn + l2*l_cn)
...
def reconstruct_spectrogram(args, assets, sample):
    ...
    with torch.no_grad():
        ...
        outputs = model(video_samples, mixed_spectrograms)
